...
class Cifar100Imbanlance(Dataset):
    def __init__(self, imbanlance_rate, num_cls=100, file_path="data/", transform=None, train=True):
    ...
    def get_per_class_num(self):
        ...
        for idx in range(self.num_cls):
            p = self.class_list[idx] / (sum(self.class_list) - self.class_list[idx])
            pro_per_cls.append(p)
        return pro_per_cls, self.class_list
